...
def load_trades(user_id: str | None = None, all_users: bool = False) -> pd.DataFrame:
    columns = [
        ...
        "confidence","status","archived","close_reason","lesson","created_at",
    ]
    sb = get_supabase()
    if sb is None:
...
if page == "Dashboard":
    ...
    new_tab, open_tab, history_tab = st.tabs(["Open a trade","Manage open trades","History"])
    ...
    st.title("What's New")
    st.subheader("Version 0.2 — Trading Workspace")
    st.markdown(
        """
        **New**
