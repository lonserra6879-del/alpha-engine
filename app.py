...
def performance_curve(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask = x["is_closed"].fillna(False).astype(bool)
    closed = (
        x.loc[mask]
        .copy()
    ...
    closed["Cumulative P&L"] = pd.to_numeric(
        closed["pnl"], errors="coerce"
    ).fillna(0).cumsum()
...
def normalize_broker_csv(raw: pd.DataFrame, broker: str, user: dict) -> tuple[pd.DataFrame, list[str]]:
    """Map common E*TRADE/Robinhood export columns into the platform trade schema."""
    df = raw.copy()
    ...
    symbol_col = find("Symbol", "Ticker", "Instrument")
    qty_col = find("Quantity", "Qty", "Shares")
    price_col = find("Price", "Average Price", "Avg Price", "Cost Per Share")
    date_col = find("Date", "Trade Date", "Activity Date", "Transaction Date")
    action_col = find("Action", "Transaction Type", "Type", "Activity")
    amount_col = find("Amount", "Net Amount", "Total")
    description_col = find("Description", "Name")

    if symbol_col is None or date_col is None:
        raise ValueError(
            "The file must contain at least a ticker/symbol column and a transaction-date column."
        )

    out = pd.DataFrame()
    out["ticker"] = df[symbol_col].astype(str).str.upper().str.strip()
    out["entry_date"] = pd.to_datetime(df[date_col], errors="coerce").dt.date.astype(str)

    if qty_col:
        out["quantity"] = pd.to_numeric(df[qty_col], errors="coerce").abs()
    else:
        out["quantity"] = np.nan
        warnings.append("Quantity column was not found.")
    ...
    elif amount_col and qty_col:
        amount = pd.to_numeric(
            df[amount_col].astype(str).str.replace(r"[$,()]", "", regex=True),
            errors="coerce",
        ).abs()
        out["entry_price"] = amount / out["quantity"].replace(0, np.nan)
        warnings.append("Entry price was estimated from amount divided by quantity.")
    else:
        out["entry_price"] = np.nan
        warnings.append("Price could not be identified.")

    action = (
        df[action_col].astype(str).str.lower()
        if action_col
        else pd.Series("buy", index=df.index)
    )
    out["raw_action"] = action
    out["status"] = np.where(
        action.str.contains("sell|sold|close", regex=True, na=False),
        "Closed",
        "Open",
    )

    # Broker exports often list buys and sells separately; import rows as case records
    # rather than pretending we can match lots perfectly.
    out["account_type"] = "Real"
    out["strategy"] = "Broker CSV Import"
    out["user_id"] = user["user_id"]
    out["owner_name"] = user["display_name"]
    out["source_broker"] = broker
    out["imported_position"] = True
    out["reason"] = "Imported from brokerage CSV"
    out["notes"] = (
        df[description_col].astype(str)
        if description_col
        else ""
    )
    out["plan_followed"] = "Yes"
    out["confidence"] = 50
    out["archived"] = False
    out["close_reason"] = np.where(
        out["status"].eq("Closed"), "Imported sale transaction", None
    )
    out["lesson"] = None
    out["exit_date"] = np.where(out["status"].eq("Closed"), out["entry_date"], None)
    out["exit_price"] = np.where(out["status"].eq("Closed"), out["entry_price"], None)
    out["stop_price"] = None
    out["target_price"] = None
    out["entry_rsi14"] = None
    out["entry_ema20"] = None
    out["entry_ema50"] = None
    out["entry_volume_ratio"] = None
    out["reconstruction_status"] = "Pending"
    out["reconstructed_at"] = None

    valid = (
        out["ticker"].ne("")
        & out["entry_date"].ne("NaT")
        & out["quantity"].fillna(0).gt(0)
        & out["entry_price"].fillna(0).gt(0)
    )
    dropped = int((~valid).sum())
    if dropped:
        warnings.append(f"{dropped} row(s) were excluded because required values were missing.")

    return out.loc[valid].reset_index(drop=True), warnings


def trade_fingerprint(row: pd.Series | dict) -> str:
    get = row.get if hasattr(row, "get") else lambda k, d=None: d
    return "|".join(
        [
            str(get("user_id", "")),
            str(get("source_broker", "")),
            str(get("ticker", "")).upper(),
            str(get("entry_date", "")),
            str(round(float(get("quantity", 0) or 0), 6)),
            str(round(float(get("entry_price", 0) or 0), 6)),
            str(get("status", "")),
        ]
    )


def batch_insert_trades(rows: pd.DataFrame, user_id: str) -> tuple[int, int]:
    existing = load_trades(user_id)
    existing_fingerprints = set()
    if not existing.empty:
        for _, row in existing.iterrows():
            existing_fingerprints.add(trade_fingerprint(row))

    added = 0
    duplicates = 0
    for _, row in rows.iterrows():
        payload = row.drop(labels=["raw_action"], errors="ignore").to_dict()
        fp = trade_fingerprint(payload)
...
def reconstruct_imported_rows(rows: pd.DataFrame) -> pd.DataFrame:
    result = rows.copy()
    snapshots = []
    for _, row in result.iterrows():
        try:
            snapshot = reconstruct_entry_snapshot(
                str(row["ticker"]),
                pd.to_datetime(row["entry_date"]).date(),
            )
        except Exception as exc:
            snapshot = {"status": "Unavailable", "message": str(exc)}
        snapshots.append(snapshot)

    result["entry_rsi14"] = [
        snap.get("rsi14") for snap in snapshots
    ]
    result["entry_ema20"] = [
        snap.get("ema20") for snap in snapshots
    ]
    result["entry_ema50"] = [
        snap.get("ema50") for snap in snapshots
    ]
    result["entry_volume_ratio"] = [
        snap.get("volume_ratio") for snap in snapshots
    ]
    result["reconstruction_status"] = [
        snap.get("status", "Unavailable") for snap in snapshots
    ]
    result["reconstructed_at"] = datetime.utcnow().isoformat()
    return result
...
with st.sidebar:
    ...
    pages = [
        "Dashboard","Trading Workspace","Import Center","Portfolio","Analytics",
...
if page == "Dashboard":
    ...
    st.title("Import Center")
    st.caption("Bring in existing positions and brokerage activity without rebuilding everything manually.")

    individual_tab, csv_tab, templates_tab = st.tabs(
        ["Existing position", "Broker CSV", "Download templates"]
    )

    with individual_tab:
        st.info(
            "Use Trading Workspace → Import existing position for one current holding. "
            "Sherlock can reconstruct the entry-date technical evidence."
        )
        if st.button("Go to Trading Workspace", use_container_width=True):
            st.session_state["requested_page"] = "Trading Workspace"
            st.warning("Select Trading Workspace in the left menu.")

    with csv_tab:
        broker = st.selectbox("Broker export format", ["E*TRADE", "Robinhood", "Generic CSV"])
        ...
        if uploaded is not None:
            try:
                ...
                normalized, warnings = normalize_broker_csv(raw, broker, user)
                for warning in warnings:
                    st.warning(warning)

                st.markdown("#### Rows ready for import")
                if normalized.empty:
                    st.error("No valid trade rows could be identified.")
                else:
                    preview_cols = [
                        "ticker", "entry_date", "quantity", "entry_price",
                        "status", "source_broker", "notes"
                    ]
                    st.dataframe(
                        normalized[preview_cols],
                        use_container_width=True,
                        hide_index=True,
                        column_config={
                            "entry_price": st.column_config.NumberColumn(format="$%.2f"),
                    ...
                    run_reconstruction = st.checkbox(
                        "Have Sherlock reconstruct RSI, EMA20, EMA50, and volume for each row",
                        value=False,
                        help="This can take longer for large CSV files.",
                    )
                    confirm = st.checkbox(
                        "I reviewed the preview and understand broker exports may require manual reconciliation."
                    )
                    ...
                    if st.button(
                        ...
                        import_rows = (
                            reconstruct_imported_rows(normalized)
                            if run_reconstruction
                            else normalized
                        )
                        added, duplicates = batch_insert_trades(
                            import_rows, user["user_id"]
                        )
                        st.success(f"Imported {added} row(s).")
                        if duplicates:
                            st.info(f"Skipped {duplicates} probable duplicate row(s).")
                        st.warning(
                            "CSV rows are imported as brokerage activity records. "
                            "Buy and sell lots may need manual matching until broker-specific reconciliation is added."
                        )
            except Exception as exc:
                st.error(f"Could not process this CSV: {exc}")

    with templates_tab:
        generic_template = pd.DataFrame(
            [
                {
                    "Date": "2026-08-01",
                    "Symbol": "NBIS",
        ...
        st.download_button(
            "Download generic import template",
            data=generic_template.to_csv(index=False).encode("utf-8"),
            file_name="londono_trade_import_template.csv",
            mime="text/csv",
            use_container_width=True,
        )
        st.caption(
            "E*TRADE and Robinhood export files may use different column names. "
            "The Import Center recognizes several common formats."
        )
    ...
    with tabs[1]:
        closed = trades.loc[trades["is_closed"].fillna(False).astype(bool)].copy()
        if closed.empty:
    ...
    with tab_closed:
        closed = trades.loc[trades["is_closed"].fillna(False).astype(bool)].copy() if not trades.empty else trades
        if closed.empty:
    ...
    st.title("What's New")
    st.subheader("Version 0.4 — Import Center")
    st.markdown(
        """
        **New**
        - Dedicated Import Center
        - E*TRADE, Robinhood, and generic CSV processing
        - Import previews before saving
        - Duplicate detection
        - Optional Sherlock reconstruction for imported rows
        - Downloadable generic CSV template

        **Fixed**
        - Dashboard no longer crashes when there are no trades
        - Analytics and Sherlock safely handle an empty journal
        - Boolean trade filters now use explicit row masks
