...
-- Version 0.3 upgrade: imported positions and reconstructed entry evidence.
alter table public.trades add column if not exists imported_position boolean not null default false;
alter table public.trades add column if not exists source_broker text;
alter table public.trades add column if not exists entry_rsi14 numeric;
alter table public.trades add column if not exists entry_ema20 numeric;
alter table public.trades add column if not exists entry_ema50 numeric;
alter table public.trades add column if not exists entry_volume_ratio numeric;
alter table public.trades add column if not exists reconstruction_status text;
alter table public.trades add column if not exists reconstructed_at timestamptz;
